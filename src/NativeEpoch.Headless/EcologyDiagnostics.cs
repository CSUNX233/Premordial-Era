...
internal static class EcologyDiagnostics
{
    public static int Run(SimulationWorld world, int steps, int reportEvery)
    {
        ...
        for (int step = 0; step < steps && world.Organisms.Count > 0; step++)
        {
            ...
            Console.WriteLine($"ecology t={s.SimulatedSeconds:F0} alive={s.Population} gen_ever={maximumGeneration} born/died_interval={s.CumulativeBirths-previousBirths}/{s.CumulativeDeaths-previousDeaths} mature_now={matureNow} mature_lacks_matter/energy={lowMatter}/{lowEnergy} matured_descendants/mutants={maturedDescendants}/{maturedMutants} minerals={s.EnvironmentMinerals:F2} detritus={s.EnvironmentDetritus:F2} stored={s.OrganismStoredMatter:F2} energy={s.LivingEnergy:F2} matter_error={s.MatterError:E3}");
        ...
        foreach (IGrouping<DeathCause, DeathRecord> group in world.RecentDeaths.GroupBy(d => d.Cause))
            Console.WriteLine($"recent_deaths={group.Key} n={group.Count()} development={group.Average(d=>d.DevelopmentCompletion):F3} energy={group.Average(d=>d.Energy):F2} substrate={group.Average(d=>d.Substrate):F3}");

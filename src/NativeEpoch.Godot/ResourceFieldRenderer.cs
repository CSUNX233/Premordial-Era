...
public sealed partial class ResourceFieldRenderer:Node3D
{
    ...
    private const int LandPlantBudget=220;
    ...
    {
        _mineralLayer=CreateLayer("MineralDeposits",new BoxMesh{Size=Vector3.One},0.86f,0.18f);
        ...
        System.Numerics.Vector2? viewCenter=null)
    {
        ...
        for(int index=0;index<snapshot.TerrainHeight.Length;index++)
        {
            ...
            Vector3 cullPoint=new(worldX-half,waterDepth>0?waterSurface:terrain,worldZ-half);
            if(!IsVisibleInViewport(camera,cullPoint))continue;
            float distance=viewCenter.HasValue
                ?System.Numerics.Vector2.Distance(viewCenter.Value,new(worldX,worldZ)):0f;
            bool distant=camera is not null&&camera.GlobalPosition.DistanceTo(cullPoint)>110f;
            int lodSlots=distant||distance>130f?1:2;
            float lodScale=distant?2.35f:1f;

            float mineral=Density(snapshot.Minerals[index],0.45);
            bool submergedMineral=waterDepth>0.05f;
            AddCellInstances(_minerals,index,0,mineral,Math.Min(2,lodSlots),spacing,
                worldX-half,submergedMineral?waterSurface+0.06f:terrain+0.14f,worldZ-half,
            ...
            AddCellInstances(_landPlants,index,1,land,waterDepth<=0?Math.Min(3,lodSlots+1):0,
                spacing,worldX-half,terrain,worldZ-half,
            ...
            AddCellInstances(_algae,index,2,algae,waterDepth>0.25f?Math.Min(2,lodSlots):0,
                spacing,worldX-half,algaeY,worldZ-half,
            ...
            float edibleY=waterDepth>0.25f
                ...
                :terrain+0.18f;
            AddCellInstances(_edible,index,3,edible,Math.Min(2,lodSlots),spacing,
                worldX-half,edibleY,worldZ-half,
            ...
            AddCellInstances(_debris,index,4,detritus,1,spacing,worldX-half,terrain+0.09f,worldZ-half,
            ...
            AddCellInstances(_debris,index,5,waste,1,spacing,worldX-half,terrain+0.12f,worldZ-half,
    ...
    private static void AddCellInstances(List<VisualInstance> target,int cell,int kind,float density,
        int maximum,float spacing,float x,float y,float z,Color color,ResourceShape shape,float lodScale)
    {
        ...
        for(int slot=0;slot<count;slot++)
        {
            ...
            Basis basis=new Basis(Vector3.Up,yaw).Scaled(scale*lodScale);
            Color instanceColor=color.Lightened(0.10f*(variation-0.82f)/0.36f);
            target.Add(new(new Transform3D(basis,new Vector3(x+offsetX,y,z+offsetZ)),
    ...
    private bool IsVisibleInViewport(Camera3D? camera,Vector3 point)
    {
        ...
        if(camera.IsPositionBehind(point))return false;
